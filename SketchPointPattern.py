...
class CommandStartingHandler(adsk.core.ApplicationCommandEventHandler):
    ...
    def notify(self, args: adsk.core.ApplicationCommandEventArgs):
        ...
        # The exact commands Fusion fires for editing timeline features
        intercept_cmds = ['BaseFeatureActivate', 'TimelineEditCommand', 'ObjectEditCommand']
        
        if cmd_id in intercept_cmds or 'edit' in cmd_id.lower():
            if ui.activeSelections.count > 0:
                selected_ent = ui.activeSelections.item(0).entity
                ...
                if isinstance(selected_ent, adsk.fusion.BaseFeature):
                    candidate = selected_ent
                elif hasattr(selected_ent, 'entity') and isinstance(selected_ent.entity, adsk.fusion.BaseFeature):
                    candidate = selected_ent.entity
